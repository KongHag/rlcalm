...
def get_vectors(doc2vec_model, corpus, vectors_size, vectors_type):
    ...
    for i in range(0, corpus_size):
        if i%100000==0:
            logging.info("Progress : %d/%d" % (i,corpus_size))
        try:
...
def format_training_sets():
    ...
    return loss_weight, weights, targets
...
def train_NN(X,Y):
    ...
    model.compile('adam',loss=BCE)#loss=custom_loss)
    history = model.fit(x=X,y=Y,validation_split=.3,verbose =2,batch_size=128,epochs=50)
    return model,history
    
def custom_loss(targets,data):
    ''' Define custom loss function for weighted BCE on 'target' column '''
    bce_loss_1 = binary_crossentropy(data[:,:1],targets[:,:1],weight=targets[:,1:2])
    bce_loss_2 = binary_crossentropy(data[:,1:],targets[:,2:])
    return (bce_loss_1 * loss_weight) + bce_loss_2 

def BCE(targets,data):
    return binary_crossentropy(data[:,:1],targets[:,:1])
...
def train_classifier(d2v, training_vectors, training_labels):
    ...
    with parallel_backend('threading'):
        ...
        rfc_random = RandomizedSearchCV(estimator = rfc, param_distributions = random_grid, n_iter = 10, cv = 3, verbose=2, random_state=42, n_jobs = WORKERS)
    ...
    validate_df = pd.DataFrame(training_predictions,columns=['target', 'male', 'female', 'homosexual_gay_or_lesbian', 'christian', 'jewish',
                        'muslim', 'black', 'white', 'psychiatric_or_mental_illness'])
...
def test_classifier(d2v_vectors, classifier, testing_vectors, testing_labels):
    logging.info("Classifier testing")
    test_vectors = d2v_vectors
    testing_predictions = classifier.predict(test_vectors)
    validate_df = train
    print("testing prediction shape",testing_predictions.shape)
    validate_df['target'] =(validate_df['target'].values>=0.5).astype(np.int)
    validate_df[GENSIM_MODEL_NAME] = testing_predictions[:,0]
    validate_df=convert_dataframe_to_bool(validate_df)
    logging.info(validate_df.head())
    bias_metrics_df = compute_bias_metrics_for_model(validate_df, identity_columns, GENSIM_MODEL_NAME, TOXICITY_COLUMN)
    performance = get_final_metric(bias_metrics_df, calculate_overall_auc(validate_df, GENSIM_MODEL_NAME))
...
train = pd.read_csv(os.path.join(path_input,'train.csv'))
loss_weight, weights, targets = format_training_sets()

def main_fun(NN=True):
    if NN:
        ...
        train_vectors = get_vectors(d2v_model, all_data, 300, 'Train')
        print("TRAINING NEURAL NET MODEL")
        model,history=train_NN(train_vectors,targets)
        ts4 = datetime.now().timestamp()
        print("NEURAL NET MODEL TRAINING EXECUTION TIME = ",int(ts4-ts3))
        test_classifier(train_vectors,model,all_data,targets)
        ts5 = datetime.now().timestamp()
        print("NEURAL NET MODEL TESTING EXECUTION TIME = ",int(ts5-ts4))
    else:
        ts1 = datetime.now().timestamp()
